...
| 实体 | 关键字段 | 关系 |
| --- | --- | --- |
| `User` | email、name、passwordHash、role、preferences | 拥有模型配置、项目、会话、MCP Server 配置和审计记录；preferences 包含布局宽度/折叠状态、明暗主题和自定义技能目录 |
...
8. 用户通过 `PATCH /auth/me/preferences` 更新布局和主题偏好，后端归一化宽度范围和主题枚举，前端同时更新本地主题缓存。
