...
- Agent 可以调用 `run_command` 请求在工作区执行命令。
- 只读工具可以直接执行。
- `create_patch` 和 `run_command` 必须进入审批流程。
- 前端应展示补丁 Diff、命令、工作目录、策略来源等预览信息。
- 用户可以批准或拒绝审批。
- 用户批准补丁后，系统应在应用前检查源文件快照，发现冲突时拒绝写入。
- 用户可以回滚已经应用且尚未被再次修改的补丁。
- 命令执行结果应记录 stdout、stderr、退出码和状态。
- 工作区运行面板应只展示当前项目实际可申请的命令，并允许用户从项目根目录或已有子目录中选择工作目录。
...
- 管理员可以配置自定义命令前缀。
- 系统应提供当前会话可用命令查询能力，便于前端提前过滤不可运行命令。
- 普通用户只能请求已启用策略内的命令。
- 管理员可以对策略外命令进行单次授权，或授权为当前项目可复用命令。
- 系统应拒绝命令链、重定向、管道、命令替换和其他高风险 shell 语法。
...
- 可测试性：关键服务应有单元测试覆盖，包括模型流式解析、Agent 工具循环、命令策略、路径沙箱、项目 Git 操作和审计查询。
- 可部署性：本地和服务器环境均可通过 Docker Compose 启动 PostgreSQL 和后端 API。
- 性能约束：文件树单层读取数量、文本文件大小、命令输出长度、命令运行时长、归档上传大小和解压大小均应设限。

## 8. 客户端与服务端职责

服务端负责认证鉴权、数据持久化、模型调用、Agent 编排、工具执行、审批、命令策略、文件沙箱、Git 操作和审计日志。

客户端负责登录注册、项目和会话选择、模型配置、消息交互、Plan 展示、工具审批、Diff 展示、代码编辑、命令运行记录、Git 状态展示和审计查询。
