...
- 当计划包含澄清问题时，用户可以提交或修订单选、多选和自定义答案，系统据此定稿并进入审查状态；无需澄清的问题可直接进入待批准状态。
- 用户可以批准处于待批准或审查状态的计划，批准时系统应保存最终计划快照；只有已批准计划才能作为 Agent 执行依据，未批准计划可以取消。
...
- TUI 应支持完整 Plan Mode 工作流，包括 `/plan` 生成草案、澄清问题作答、答案保存、计划定稿、审查批准、取消和已批准计划执行。
...
- 可测试性：关键服务应有单元测试覆盖，包括模型流式解析、Plan Mode 生命周期与幂等创建、Agent 工具循环、命令策略、会话命令授权、命令运行环境提示、路径沙箱、local workspace 创建/删除保护、项目 Git 操作和审计查询；TUI 应覆盖启动、登录提示、API 不可达、SSE 解析、历史会话切换、Plan 问答/审查、审批动作和文件树忽略规则。
...
服务端负责认证鉴权、系统能力声明、数据持久化、项目工作区模型、Plan Mode 生命周期、模型调用、Agent 编排、工具执行、审批、命令策略、会话命令授权、命令运行环境提示、文件沙箱、Git 操作和审计日志。

Web 客户端负责登录注册、managed 项目创建导入、项目和会话选择、模型配置、消息交互、Plan 展示与批准、工具审批、Diff 展示、代码编辑、命令授权状态、命令运行记录、Git 状态展示和审计查询。

TUI 客户端负责终端登录配置、本机或远程 API 连接、local 项目绑定、会话启动、SSE 流式消息展示、Plan 问答与审查、审批交互、Diff 预览、命令预览和文件树浏览。
