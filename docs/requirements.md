...
- 用户注册登录、角色和界面偏好管理。
- 已登录用户可以通过 Web 端提交当前密码和新密码来修改登录密码。
...
| Codex | 强调 Plan Mode、工具调用和审批 | 产品形态和本项目交付目标不同 | 将 Agent 工作流落到可解释、可建模的项目系统中 |
...
- 用户可以先请求注册邮箱验证码，再通过邮箱、昵称、密码和 6 位验证码注册账号。
- 注册验证码应通过 SMTP 邮件发送，10 分钟内有效，支持重发冷却、单邮箱频率限制、全局发送上限和最大校验次数限制。
- 用户可以登录并获得 JWT，用于后续访问受保护接口。
- 系统支持管理员邀请码，输入正确邀请码的注册用户获得管理员角色。
- 用户可以查询当前账号信息。
- 用户可以在 Web 账户安全页提交当前密码、新密码和确认密码来修改登录密码；后端必须校验当前密码正确，禁止新密码与旧密码相同，并使用 bcrypt 重新哈希保存新密码。
...
- Web 客户端应提供登录、注册、工作区、账户安全、模型设置、命令权限设置和审计日志页面。
- 账户安全页应展示当前账号昵称或邮箱，并提供修改密码表单；前端应校验两次新密码一致，调用受 JWT 保护的 `PATCH /auth/me/password`，成功后清空密码输入并给出明确反馈。
...
- 用户可以在 Android 客户端登录，安全保存 API 地址、JWT 和用户名称；debug 和 release 构建默认公网 API 地址为 `http://182.92.150.169/api`，本地模拟器开发时用户可在登录页改为 `http://10.0.2.2:3000/api`，用户仍可在登录页或设置页修改。
...
服务端负责认证鉴权、邮箱验证码、当前账号查询、密码修改、系统能力声明、移动端概览聚合、数据持久化、项目工作区模型、仓库级 `AGENTS.md` 指令读取与生成、Plan Mode 生命周期、计划讨论与修订、模型调用、Agent 编排（含活动技能注入、项目指令注入、Web 检索、MCP 工具暴露和未知工具名校验）、工具执行、审批、回合撤销/重做、命令策略、会话命令授权、命令运行环境提示、文件沙箱、Git 操作和审计日志。

Web 客户端负责登录注册、账户安全和密码修改、managed 项目创建导入、项目和会话选择、模型配置、消息交互、Markdown/数学公式渲染、Build/Plan 输入模式、Plan 展示、批准与执行、回合撤销/重做、回合完成后 transcript 重载、工具审批、Diff 展示、代码编辑、命令授权状态、命令运行记录、Git 状态展示和审计查询。
