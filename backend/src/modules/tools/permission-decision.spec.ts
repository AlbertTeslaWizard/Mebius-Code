...
describe('permission decision', () => {
  ...
  it('keeps read_only strict for network commands', () => {
    expect(decidePermission(PermissionMode.ReadOnly, commandRequest('curl https://example.com'))).toBe('deny');
  });
  ...
  it('uses session approval rules before mode defaults while keeping external command paths guarded', () => {
    ...
    expect(decidePermission(PermissionMode.AskFirst, commandRequest('npm test -- --watch'), [rule])).toBe('allow');
...
function inspectCommand(command: string): CommandInspection {
  ...
  const shellTokens = ['&&', '||', ';', '|', '>', '<'].filter((token) => normalized.includes(token));
  const parts = normalized.match(/(?:[^\s"]+|"[^"]*")+/g)?.map((part) => part.replace(/^"|"$/g, '')) ?? [];
